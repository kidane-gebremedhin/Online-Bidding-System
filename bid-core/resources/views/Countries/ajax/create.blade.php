<div class="col-md-12">
 <div class="col-md-8">
   {{Form::open(array("route"=>"countries.store", "method"=>"POST", "class"=>"post"))}}
   <label class="nextUrl" nextUrl="{{route('countries.index')}}" />
   <div class="col-md-12 panel panel-success">
    <div class="panel-heading">
     <h4>{{App\Global_var::getLangString('Create_New_Country', $language_strings)}} <a href="{{route('countries.index')}}" class="get btn btn-success btn-xs pull-right" nextUrl="{{route('countries.index')}}"><i class="fa fa-eye"></i> <strong>{{App\Global_var::getLangString('List', $language_strings)}}</strong></a> </h4>
   </div>
