<div class="col-md-12">
  <div class="col-md-8">
   {!!Form::model($wereda, array("route"=>["weredas.update", $wereda->id], "method"=>"POST", "class"=>"post"))!!}
   <label class="nextUrl" nextUrl="{{route('weredas.index')}}" />
   <div class="panel panel-success">
    <div class="panel-heading">
     <h4> {{App\Global_var::getLangString('Wereda', $language_strings)}} {{App\Global_var::getLangString('Edit', $language_strings)}} 
       <a href="{{route('weredas.index')}}" class="get btn btn-success btn-sm pull-right" nextUrl="{{route('weredas.index')}}"><i class="fa fa-eye"></i> <strong>{{App\Global_var::getLangString('List', $language_strings)}}</strong></a> </h4>
     </div>           
