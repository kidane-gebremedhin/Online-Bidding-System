...
<div class="col-md-12 panel-body">
  ...
  <div class="col-md-12">
    ...
    @if($user->isSubscriber())
      ...
      {{ Form::select('businessCategoryId', [null=>'-- --']+$business_categories, $user->businessCategoryId, array('class'=>'businessCategoryId select2 form-control', 'required'=>'true'))}}<br>
      {{App\Global_var::getLangString('Subscription_Plan', $language_strings)}}
      {{ Form::select('subscriptionPlanId', [null=>'-- --']+$subscription_plans, $user->subscriptionPlanId, array('class'=>'subscriptionPlanId select2 form-control', 'required'=>'true'))}}<br>
      {{App\Global_var::getLangString('Company_Name', $language_strings)}}
      {{ Form::text('companyName', $user->companyName, array('class'=>'form-control', 'required'=>'true'))}}<br>
    @endif
      <!-- {{App\Global_var::getLangString('Username', $language_strings)}}
      {{ Form::text('userName', $user->userName, array('class'=>'form-control', 'required'=>'true'))}}<br> -->
      {{App\Global_var::getLangString('Full_Name', $language_strings)}}
      {{ Form::text('fullName', $user->fullName, array('class'=>'form-control', 'required'=>'true'))}}<br>
      {{App\Global_var::getLangString('Email', $language_strings)}}
      {{ Form::email('email', $user->email, array('class'=>'form-control', 'required'=>'true'))}}<br>
